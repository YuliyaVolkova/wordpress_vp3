...
if (!function_exists('turnews_thumbnail')) {
    function turnews_thumbnail()
    {
...
if (!function_exists('right_sidebare_widgets_init')) {
    function right_sidebare_widgets_init()
    {
    ...
    add_action('widgets_init', 'right_sidebare_widgets_init');
}

// for actions single page
if (!function_exists('categoriesAction')) {
    function categoriesAction($id, $taxonomy)
    {
        ...
        foreach ($terms as $term) {
            if (in_array($term->name, $termsNameAr)) {
                continue;
            }
            $termsNameAr[] = $term->name;
            $strTerms = $strTerms . '<span class="postmetadata_item"><a class="btn-read-post" href="' . get_term_link($term, $taxonomy) . '">'. $term->name .'</a></span>';
        }
        return $strTerms;
    }
}
 ...
 */
if (!function_exists('wpdocs_filter_wp_title')) {
...
if (!function_exists('themeslug_enqueue_style')) {
...
//shortcode function for add current Year from admin-panel
